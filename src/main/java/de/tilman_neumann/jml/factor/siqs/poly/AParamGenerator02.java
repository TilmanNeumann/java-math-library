 ...
 */
public class AParamGenerator02 implements AParamGenerator {
	private static final Logger LOG = LogManager.getLogger(AParamGenerator02.class);
	 ...
	 */
	public AParamGenerator02(Integer wanted_qCount) {
	...
	private int estimateQCount(int bits) {
		// Though qCount=3 is slightly faster for N<=80 bit, we start with qCount=4 because it is most stable for small N
		if (bits<100) return 4;
		if (bits<117) return 5;
		if (bits<136) return 6;
		if (bits<155) return 7;
		if (bits<185) return 8;
		if (bits<218) return 9;
		if (bits<260) return 10;
		if (bits<305) return 11;
		if (bits<370) return 12; // bound estimated from data up to 360 bit
		if (bits<470) return 13; // extrapolation
		return 14;
	}
