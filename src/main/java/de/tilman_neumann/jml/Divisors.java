...
public class Divisors {
	...
	private static ArrayList<BigInteger> getDivisors_v2(BigInteger n) {
		...
		// copy small divisors
		ArrayList<BigInteger> allDivisors = new ArrayList<>(smallDivisors);
		...
		while (!stack.isEmpty()) {
			...
			if (divisors.add(divisor)) {
				for (int i=0; i<powers.size(); i++) {
					int power = powers.get(i);
					// create new entry
					ArrayList<Integer> reducedPowers = new ArrayList<>(powers); // copy
	...
	public static SortedSet<BigInteger> getDivisors/*BottomUp*/(SortedMap<BigInteger, Integer> factors) {
		...
		Stack<ArrayList<Integer>> stack = new Stack<>();
		ArrayList<Integer> emptyPowers = new ArrayList<>();
		...
		while (!stack.isEmpty()) {
			...
			if (divisors.add(divisor)) {
				for (int i=0; i<maxPowers.size(); i++) {
					...
					if (power < maxPower) {
						// create new entry
						ArrayList<Integer> enhancedPowers = new ArrayList<>(powers); // copy
	...
	public static ArrayList<BigInteger> getSmallDivisors_v1(BigInteger n) {
		...
		ArrayList<BigInteger> smallDivisors = new ArrayList<>();
	...
	public static BigInteger sumOfDivisors(SortedMap<BigInteger, Integer> factors) {
		ArrayList<BigInteger> entrySums = new ArrayList<>();
