...
public class SIQS_Small extends FactorAlgorithm {
	...
	public SIQS_Small(
			...
			Sieve sieve, TDiv_QS auxFactorizer, int extraCongruences, boolean useLegacyFactoring, boolean searchSmallFactors) {
	...
	private BigInteger findSingleFactorInternal(BigInteger N) {
		...
		try {
			while (true) {
				...
				if (DEBUG) {
					if (DEBUG) LOG.debug(polyGenerator.getName() + ": " + polyGenerator.getReport().getOperationDetails());
