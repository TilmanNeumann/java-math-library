 ...
 */
public class TDiv_QS_2Large_UBI_BarrettD implements TDiv_QS {
	private static final Logger LOG = Logger.getLogger(TDiv_QS_2Large_UBI_BarrettD.class);
	private static final boolean DEBUG = false;

	private static final double CAST_CORRECTION = 1.0 / (1L << 32);
	...
	public void initializeForAParameter(BigInteger da, int d, BigInteger b, SolutionArrays solutionArrays, int filteredBaseSize, int[] unsievedBaseElements) {
		...
		pinvArrayD = solutionArrays.pinvArrayD;
	...
	private AQPair test(BigInteger A, BigInteger Q, int x) {
		...
		for (int pIndex = baseSize-1; pIndex > 0; pIndex--) { // p[0]=2 was already tested
			...
			if (xAbs<p) {
				...
				// Compute x%p using double-valued Barrett reduction, see https://en.wikipedia.org/wiki/Barrett_reduction.
				final long q1 = (long) (x*pinvArrayD[pIndex] + CAST_CORRECTION);
				xModP = (int) (x - q1 * p);
		...
		if (QRestBits<50) {
			...
			if (DEBUG) LOG.debug("test(): pMax^2 = " + pMaxSquare + ", QRest = " + QRest + " (" + QRestBits + " bits) not prime -> use pollardRhoR64Mul63");
			factor1 = pollardRhoR64Mul63.findSingleFactor(QRest);
		} else if (QRestBits<63) {
			if (DEBUG) LOG.debug("test(): pMax^2 = " + pMaxSquare + ", QRest = " + QRest + " (" + QRestBits + " bits) not prime -> use pollardRho64");
		...
		if (DEBUG) LOG.debug("test(): QRest = " + QRest + " (" + QRestBits + " bits) = " + factor1 + " * " + factor2);
