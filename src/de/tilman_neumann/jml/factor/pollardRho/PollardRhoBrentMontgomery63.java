 ...
 * 
 * The main reason why Montgomery multiplication could be helpful is that no conversions to/from Montgomery form are required.
 * This implementation is long-based and uses the Montgomery reducer R=2^63. Pretty fast.
 * 
...
public class PollardRhoBrentMontgomery63 extends FactorAlgorithmBase {
	...
	private static final int Rbits = 63;
	private static final long RMask = ~R; // R-1 = 0x7FFFFFFFFFFFFFFFL, helps to compute x mod R = x & (R - 1)
	private static final long Rhalf = 1L << 62;
	...
	private long montgomeryMult(final long a, final long b) {
		...
		// t = ab * (-1/N) mod R
		long t = Int127.mul64(ab.and(RMask), minusNInvModR).and(RMask);
		long reduced = ab.add(Int127.mul64(t, N)).shiftRight(Rbits).getLow();
