...
public class Hart_Fast2 extends FactorAlgorithm {
	...
	private static final int I_MAX = (int) Math.pow(MAX_N, K_LIMIT_EXP);
	...
	static {
		...
		System.out.println("Hart_Fast2: Initialized sqrt array with " + I_MAX + " entries");
	}
	...
	public long findSingleFactor(long N) {
		...
		int k = K_MULT;
		try {
			// having a limit here makes Hart's algorithm a prime test. But it is bad for small N performance...
			int iMax = (int) Math.pow(N, K_LIMIT_EXP);
			for (int i=1; i<=iMax;) {
