 ...
 * Works for N <= 45 bit.
 * 
 * This version does trial division before the main loop.
 * 
...
public class Lehman_TDivFirst extends FactorAlgorithmBase {
	...
	public long findSingleFactor(long N) {
		...
		// 3. continue main loop for larger even k, where we can have only 1 a-value per k
		int k1 = k;
		for ( ; k1 <= kLimit; k1+=2) {
		...
		// 4. continue main loop for larger odd k, where we can have only 1 a-value per k
		int k2 = k+1;
		for ( ; k2 <= kLimit; k2+=2) {
