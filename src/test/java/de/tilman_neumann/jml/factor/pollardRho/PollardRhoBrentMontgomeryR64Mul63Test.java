...
/**
 * QA tests for the PollardRhoBrentMontgomery64 factor algorithm.
 * 
 * All tests successful, but some 62,63 bit numbers with questionable performance.
 */
public class PollardRhoBrentMontgomeryR64Mul63Test {
	private static final Logger LOG = LogManager.getLogger(PollardRhoBrentMontgomeryR64Mul63Test.class);

	private static final boolean RUN_SLOW_TESTS_TOO = false;

	private static PollardRhoBrentMontgomeryR64Mul63 pollardRho;

	@BeforeClass
	public static void setup() {
		ConfigUtil.initProject();
		pollardRho = new PollardRhoBrentMontgomeryR64Mul63();
	}
	...
	public void testHarderSemiprimesWithFactorsOfDifferentSize() {
		...
		if (RUN_SLOW_TESTS_TOO) assertFactorizationSuccess(4085731848127832849L, "825205747 * 4951167467"); // 62 bit
		...
		if (RUN_SLOW_TESTS_TOO) assertFactorizationSuccess(7836704265571283783L, "130781947 * 59921911589"); // 63 bit
		if (RUN_SLOW_TESTS_TOO) assertFactorizationSuccess(8940500625246794041L, "240556271 * 37165942871"); // 63 bit
		assertFactorizationSuccess(3608228875180849937L, "49696057 * 72605938841"); // 62 bit
		if (RUN_SLOW_TESTS_TOO) assertFactorizationSuccess(9170754184293724117L, "290060959 * 31616644363"); // 63 bit
	}
