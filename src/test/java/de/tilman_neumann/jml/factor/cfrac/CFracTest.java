...
import de.tilman_neumann.jml.factor.cfrac.tdiv.TDiv_CF01;
 ...
 * CFrac tests.
 * 
 * CFrac is an algorithm designed to find a single factor of "big" numbers. (as big as "big" could mean when the algorithm was invented, but nowadays it's not that big anymore)
 * It is not meant to find the complete factorization of arbitrary composite numbers without help of other algorithms that sort out the factorization of very small products.
 * 
 * As such, the relevant piece to test here is the findSingleFactor() method.
 * The lower end of the size of inputs where this method starts to work reliably depends on the ks_adjust parameter.
 * With ks_adjust==5 as used here it will work for N>=24 bit.
 * 
 * Some tests here demonstrate that CFrac is capable of finding the full factorization of <em>some</em> numbers containing several small factors;
 * but there is no guarantee for that.
 */
public class CFracTest {
	private static final Logger LOG = LogManager.getLogger(CFracTest.class);

	private static final CFrac cFrac = new CFrac(true, 5, 1.5F, 0.152F, 0.253F, new TDiv_CF01(), new MatrixSolverGauss02(), 5);
	...
	@Test
	public void testLowEndSemiprimes() {
		List<BigInteger> fails = FactorTestInfrastructure.testSemiprimes(BigInteger.valueOf(1<<24), BigInteger.valueOf((1<<24) + 10000), cFrac);
	...
	public void testSomeInputs() {
		...
		assertFactorizationError("5679148659138759837165981543", "3^3 * 466932157 * 450469808245315337");
		// too big for CFrac
	...
	private void assertFactorizationSuccess(long N, String expectedPrimeFactorizationStr) {
		SortedMultiset<BigInteger> factors = cFrac.factor(BigInteger.valueOf(N));
	...
	private void assertFactorizationSuccess(String oddNStr, String expectedPrimeFactorizationStr) {
		...
		SortedMultiset<BigInteger> factorResult = cFrac.factor(N);
		assertEquals(expectedPrimeFactorizationStr, factorResult.toString("*", "^"));
		t1 = System.currentTimeMillis();
		LOG.info("Factoring " + oddNStr + " took " + (t1-t0) + "ms");
	}

	private void assertFactorizationError(String oddNStr, String expectedPrimeFactorizationStr) {
		long t0, t1;
		t0 = System.currentTimeMillis();
		BigInteger N = new BigInteger(oddNStr);
		SortedMultiset<BigInteger> factorResult = cFrac.factor(N);
