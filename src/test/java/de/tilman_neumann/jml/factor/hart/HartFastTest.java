...
public class HartFastTest {
	...
	private static HartFast hart = new HartFast(false);
	...
	public void testHarderSemiprimesWithFactorsOfSimilarSize() {
		...
		assertFactorizationError(3225273260887418687L, "567914891 * 5679148957"); // 62 bit, does not even run with I_MAX=2^25
	}
	
	@Test
	public void testHarderSemiprimesWithFactorsOfDifferentSize() {
		assertFactorizationError(17977882519205951L, "6026521 * 2983127831"); // 54 bit, needs I_MAX=2^22
		assertFactorizationError(57410188984551071L, "7419119 * 7738141009"); // 56 bit, needs I_MAX=2^22
		assertFactorizationError(708198179721093877L, "31472003 * 22502481959"); // 60 bit, needs I_MAX=2^23
		assertFactorizationError(4085731848127832849L, "825205747 * 4951167467"); // 62 bit, needs I_MAX=2^22
		
		assertFactorizationError(873351084013120721L, "29133103 * 29977963007"); // 60 bit, does not even run with I_MAX=2^25
		assertFactorizationError(7355428158429213199L, "6226303 * 1181347608433"); // 63 bit, needs I_MAX=2^23
		assertFactorizationError(7836704265571283783L, "130781947 * 59921911589"); // 63 bit, needs I_MAX=2^23
		assertFactorizationError(8940500625246794041L, "240556271 * 37165942871"); // 63 bit, needs I_MAX=2^23
		assertFactorizationError(3608228875180849937L, "49696057 * 72605938841"); // 62 bit, does not even run with I_MAX=2^25
		assertFactorizationError(9170754184293724117L, "290060959 * 31616644363"); // 63 bit, does not even run with I_MAX=2^25
	}
