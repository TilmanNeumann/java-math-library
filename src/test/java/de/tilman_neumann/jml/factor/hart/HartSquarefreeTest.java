...
public class HartSquarefreeTest extends FactorTestBase {
	...
	public void testHarderSemiprimesWithFactorsOfSimilarSize() {
		assertFullFactorizationFailure(583452201186457L, "12077393 * 48309449"); // 50 bit, needs I_MAX=2^23
		assertFullFactorizationFailure(978446392056317L, "15640063 * 62560259"); // 50 bit, needs I_MAX=2^24
		assertFullFactorizationFailure(1001714888187367L, "15824917 * 63299851"); // 50 bit, needs I_MAX=2^22
		...
		assertFullFactorizationFailure(19780076027096071L, "125793713 * 157242167"); // 55 bit, needs I_MAX=2^24
		assertFullFactorizationFailure(23667184817186563L, "133230703 * 177640621"); // 55 bit, needs I_MAX=2^23
		assertFullFactorizationFailure(21560613770598187L, "127162963 * 169551049"); // 55 bit, needs I_MAX=2^22
		assertFullFactorizationFailure(27244608402201133L, "130490851 * 208785583"); // 55 bit, needs I_MAX=2^22
		assertFullFactorizationFailure(19574813545806263L, "125139401 * 156424063"); // 55 bit, needs I_MAX=2^23
	...
	public void testHarderSemiprimesWithFactorsOfDifferentSize() {
		assertFullFactorizationFailure(17977882519205951L, "6026521 * 2983127831"); // 54 bit, needs I_MAX=2^22
		assertFullFactorizationSuccess(57410188984551071L, "7419119 * 7738141009"); // 56 bit
		assertFullFactorizationFailure(708198179721093877L, "31472003 * 22502481959"); // 60 bit, needs I_MAX=2^22
		assertFullFactorizationSuccess(4085731848127832849L, "825205747 * 4951167467"); // 62 bit
		
		assertFullFactorizationFailure(873351084013120721L, "29133103 * 29977963007"); // 60 bit, needs I_MAX=2^25
		assertFullFactorizationSuccess(7355428158429213199L, "6226303 * 1181347608433"); // 63 bit, needs I_MAX=2^21
		assertFullFactorizationFailure(7836704265571283783L, "130781947 * 59921911589"); // 63 bit, needs I_MAX=2^22
		assertFullFactorizationFailure(8940500625246794041L, "240556271 * 37165942871"); // 63 bit, needs I_MAX=2^23
