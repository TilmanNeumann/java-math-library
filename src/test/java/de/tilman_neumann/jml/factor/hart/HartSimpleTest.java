...
public class HartSimpleTest extends FactorTestBase {
	...
	public void testCompositesWithSmallFactors() {
		...
		assertFullFactorizationFailure(94602505, "5 * 18920501"); // 27 bit, needs trial division
		assertFullFactorizationSuccess(258555555, "3^2 * 5 * 5745679"); // 28 bit
		assertFullFactorizationFailure(436396385, "5 * 87279277"); // 29 bit, needs trial division
		assertFullFactorizationFailure(612066705, "3 * 5 * 40804447"); // 30 bit, needs trial division
		assertFullFactorizationFailure(2017001503, "11 * 183363773"); // 31 bit, needs trial division
		...
		assertFullFactorizationFailure(16032993843L, "3 * 5344331281"); // 34 bit, needs trial division
		assertFullFactorizationFailure(26036808587L, "83 * 313696489"); // 35 bit, needs trial division
		assertFullFactorizationFailure(41703657595L, "5 * 8340731519"); // 36 bit, needs trial division
		assertFullFactorizationSuccess(68889614021L, "19 * 43 * 84320213"); // 37 bit
		assertFullFactorizationFailure(197397887859L, "3^2 * 21933098651"); // 38 bit, needs trial division
	...
	public void testCompositesWithManyFactors() {
		...
		assertFullFactorizationFailure(1096954293075013905L, "3 * 5 * 7^2 * 169681 * 8795650783"); // 60 bit, needs trial division
	...
	public void testSquares() {
		...
		assertFullFactorizationFailure(1000000014000000049L, "1000000007^2"); // 60 bit // does not even work with I_MAX=2^25
	}
	...
	public void testSemiprimesWithFactorsOfDifferentSize() {
		...
		assertFullFactorizationFailure(737091621253457L, "1772291 * 415897627"); // 50 bit, needs I_MAX=2^22
		assertFullFactorizationSuccess(1112268234497993L, "68053 * 16344146981"); // 50 bit
		assertFullFactorizationFailure(2986396307326613L, "27054757 * 110383409"); // 52 bit, needs I_MAX=2^23
		...
		assertFullFactorizationFailure(209195243701823L, "248231 * 842744233"); // 48 bit, needs I_MAX=2^23
		assertFullFactorizationSuccess(290236682491211L, "12819941 * 22639471"); // 49 bit
		assertFullFactorizationFailure(485069046631849L, "287537 * 1686979577"); // 49 bit, needs I_MAX=2^22
		assertFullFactorizationSuccess(1239671094365611L, "5148659 * 240775529"); // 51 bit
		assertFullFactorizationFailure(2815471543494793L, "3865469 * 728364797"); // 52 bit, needs I_MAX=2^24
	...
	public void testHarderSemiprimesWithFactorsOfSimilarSize() {
		...
		assertFullFactorizationFailure(3225273260887418687L, "567914891 * 5679148957"); // 62 bit // does not even work with I_MAX=2^25
	}
	
	@Test
	public void testHarderSemiprimesWithFactorsOfDifferentSize() {
		assertFullFactorizationFailure(17977882519205951L, "6026521 * 2983127831"); // 54 bit // does not even work with I_MAX=2^25
		assertFullFactorizationFailure(57410188984551071L, "7419119 * 7738141009"); // 56 bit // does not even work with I_MAX=2^25
		assertFullFactorizationFailure(708198179721093877L, "31472003 * 22502481959"); // 60 bit // does not even work with I_MAX=2^25
		assertFullFactorizationSuccess(4085731848127832849L, "825205747 * 4951167467"); // 62 bit
		
		assertFullFactorizationFailure(873351084013120721L, "29133103 * 29977963007"); // 60 bit // does not even work with I_MAX=2^25
		assertFullFactorizationFailure(7355428158429213199L, "6226303 * 1181347608433"); // 63 bit // does not even work with I_MAX=2^25
		assertFullFactorizationFailure(7836704265571283783L, "130781947 * 59921911589"); // 63 bit // does not even work with I_MAX=2^25
		assertFullFactorizationSuccess(8940500625246794041L, "240556271 * 37165942871"); // 63 bit
		assertFullFactorizationFailure(3608228875180849937L, "49696057 * 72605938841"); // 62 bit // does not even work with I_MAX=2^25
