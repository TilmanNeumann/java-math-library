...
/**
 * QA tests for the HartSimple factor algorithm.
 * 
 * This is the simplest Hart implementation kept here for reference only. Don't use it!
 * The factorization errors in this test point out that it is much worse than other variants.
 */
public class HartSimpleTest {
	private static final Logger LOG = LogManager.getLogger(HartSimpleTest.class);

	private static final HartSimple hart = new HartSimple();
	...
	public void testSmallestComposites() {
		List<Integer> fails = FactorTestInfrastructure.testSmallComposites(100000, hart);
	...
	public void testCompositesWithSmallFactors() {
		...
		assertFactorizationError(94602505, "5 * 18920501"); // 27 bit, needs trial division
		assertFactorizationSuccess(258555555, "3^2 * 5 * 5745679"); // 28 bit
		assertFactorizationError(436396385, "5 * 87279277"); // 29 bit, needs trial division
		assertFactorizationError(612066705, "3 * 5 * 40804447"); // 30 bit, needs trial division
		assertFactorizationError(2017001503, "11 * 183363773"); // 31 bit, needs trial division
		assertFactorizationSuccess(3084734169L, "3^2 * 11 * 31158931"); // 32 bit
		assertFactorizationSuccess(6700794123L, "3 * 41 * 54478001"); // 33 bit
		assertFactorizationError(16032993843L, "3 * 5344331281"); // 34 bit, needs trial division
		assertFactorizationError(26036808587L, "83 * 313696489"); // 35 bit, needs trial division
		assertFactorizationError(41703657595L, "5 * 8340731519"); // 36 bit, needs trial division
		assertFactorizationSuccess(68889614021L, "19 * 43 * 84320213"); // 37 bit
		assertFactorizationError(197397887859L, "3^2 * 21933098651"); // 38 bit, needs trial division
		...
		assertFactorizationError(624800360363L, "233 * 2681546611"); // 40 bit, needs trial division
		assertFactorizationError(883246601513L, "251 * 3518910763"); // 40 bit, needs trial division
	}

	@Test
	public void testCompositesWithManyFactors() {
		assertFactorizationSuccess(35184372094495L, "5 * 13^2 * 17 * 19 * 29 * 47 * 271 * 349"); // 46 bit
		assertFactorizationError(1096954293075013905L, "3 * 5 * 7^2 * 169681 * 8795650783"); // 60 bit, needs trial division
	...
	public void testSquares() {
		...
		assertFactorizationError(1000000014000000049L, "1000000007^2"); // 60 bit // does not even work with I_MAX=2^25
	}
	...
	public void testSemiprimesWithFactorsOfDifferentSize() {
		...
		assertFactorizationError(737091621253457L, "1772291 * 415897627"); // 50 bit, needs I_MAX=2^22
		assertFactorizationSuccess(1112268234497993L, "68053 * 16344146981"); // 50 bit
		assertFactorizationError(2986396307326613L, "27054757 * 110383409"); // 52 bit, needs I_MAX=2^23
		...
		assertFactorizationError(209195243701823L, "248231 * 842744233"); // 48 bit, needs I_MAX=2^23
		assertFactorizationSuccess(290236682491211L, "12819941 * 22639471"); // 49 bit
		assertFactorizationError(485069046631849L, "287537 * 1686979577"); // 49 bit, needs I_MAX=2^22
		assertFactorizationSuccess(1239671094365611L, "5148659 * 240775529"); // 51 bit
		assertFactorizationError(2815471543494793L, "3865469 * 728364797"); // 52 bit, needs I_MAX=2^24
	...
	public void testHarderSemiprimesWithFactorsOfSimilarSize() {
		assertFactorizationError(1454149122259871L, "26970011 * 53917261"); // 51 bit, needs I_MAX=2^22
		assertFactorizationError(5963992216323061L, "54599437 * 109231753"); // 53 bit, needs I_MAX=2^23
		assertFactorizationError(26071073737844227L, "114161413 * 228370279"); // 55 bit, needs I_MAX=2^24
		assertFactorizationError(8296707175249091L, "64415917 * 128799023"); // 53 bit, needs I_MAX=2^23
		assertFactorizationError(35688516583284121L, "133595303 * 267139007"); // 55 bit, needs I_MAX=2^24
		assertFactorizationError(35245060305489557L, "132737413 * 265524689"); // 55 bit, needs I_MAX=2^24
	...
	public void testHarderSemiprimesWithFactorsOfDifferentSize() {
		assertFactorizationError(17977882519205951L, "6026521 * 2983127831"); // 54 bit // does not even work with I_MAX=2^25
		assertFactorizationError(57410188984551071L, "7419119 * 7738141009"); // 56 bit // does not even work with I_MAX=2^25
		assertFactorizationError(708198179721093877L, "31472003 * 22502481959"); // 60 bit // does not even work with I_MAX=2^25
		assertFactorizationSuccess(4085731848127832849L, "825205747 * 4951167467"); // 62 bit
		
		assertFactorizationError(873351084013120721L, "29133103 * 29977963007"); // 60 bit // does not even work with I_MAX=2^25
		assertFactorizationError(7355428158429213199L, "6226303 * 1181347608433"); // 63 bit // does not even work with I_MAX=2^25
		assertFactorizationError(7836704265571283783L, "130781947 * 59921911589"); // 63 bit // does not even work with I_MAX=2^25
	...
	private void assertFactorizationSuccess(long N, String expectedPrimeFactorizationStr) {
		...
		SortedMultiset<BigInteger> factors = hart.factor(NBig);
